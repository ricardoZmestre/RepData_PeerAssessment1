...
#msteps <- lm(log(steps+1) ~ scale(interval) + I(scale(interval)^2) + 
#               factor(weekdays(df$date)) - 1, data=df, na.action=na.exclude)
#kable(summary(msteps)$coeff)
#fit <- predict(msteps, df)
#dfnan <- df
#dfnan$steps[is.na(df$steps)] <- exp(fit[is.na(df$steps)])-1
...
```{r table NA allocation, results='asis'}
### Question 3
